...
df = df.drop(columns = ['age','availability'])
df = df.T

data = combat(df,availability ,age, 'age').T
data['availability'] = availability
data.insert(loc = 0, column = 'age', value = age['age'])
data.insert(loc = 0, column = 'patient', value = availability )
data.to_csv("after_combat.txt",sep=',',index = False)
